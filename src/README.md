...
Use it: open **`site/index.html`** — one self-contained file (UI + data + `.hkp` parser, all inlined).
It runs entirely in the browser: host it on any static site, or just open the file locally. No
server, no install, no network.

Build it: `site/index.html` is generated from `page.html` + the data files by
...
`site/index.html` — `page.html` itself also works that way via a dev fallback that fetches the json.
...
- **`site/index.html`** — the **built** product: `page.html` with the three data files inlined.
  Generated by `python3 hotkey_editor.py --build`; this folder is what you deploy (e.g. point a
  static host's output directory at `site/`). Don't edit by hand.
- **`hotkey_editor.py`** — build/data CLI (no server): `--build` assembles `site/index.html`,
